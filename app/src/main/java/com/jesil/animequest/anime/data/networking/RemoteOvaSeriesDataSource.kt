...
import com.jesil.animequest.core.domain.utils.QueryConstants.OVA_SERIES
...
class RemoteOvaSeriesDataSource(
    private val httpClient: HttpClient
) : OvaSeriesDataSource {

    override suspend fun getOvaSeries(): Result<List<Anime>, NetworkError> =
        safeCall<AnimeResponseDto> {
            httpClient.get(
                urlString = constructUrl(url = RANKING_ANIME_ENDPOINT)
            ) {
                parameter("ranking_type", OVA_SERIES)
            }
        }.map { response ->
