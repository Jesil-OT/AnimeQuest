...
import com.jesil.animequest.core.domain.utils.QueryConstants.TOP_AIRING
...
class RemoteTopAiringDataSource(
    private val httpClient: HttpClient
) : TopAiringDataSource {
    override suspend fun getTopAiringAnime(): Result<List<Anime>, NetworkError> =
        safeCall<AnimeResponseDto> {
            httpClient.get(
                urlString = constructUrl(url = RANKING_ANIME_ENDPOINT)
            ) {
                parameter("ranking_type", TOP_AIRING)
            }
        }.map { response ->
